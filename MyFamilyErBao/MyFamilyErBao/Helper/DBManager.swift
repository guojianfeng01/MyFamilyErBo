...
//  MyFamilyErBao
//
//  Created by guojianfeng on 2017/11/17.
...
let DB_NAME = "t_image.sqlite"
let SAFE_KEY = "t_image_101"
let SQLITE_TEXT_TYPE = "TEXT"
let SQLITE_INT_TYPE = "TEXT"
...
class DBManager: NSObject {
    ...
    func dbBasePath() -> String {
        let fileName = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)
        let path = fileName.last! + "/" + DB_NAME
        return path
    }
    ...
    @discardableResult private func openDB() -> Bool{
        if db == nil {
            ...
            sqlite3_busy_handler(db, { (ptr,count) -> Int32 in
